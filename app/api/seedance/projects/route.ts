...
export async function GET(request: NextRequest) {
  ...
  const projects = await prisma.videoProject.findMany({
    ...
    take: 100,
    include: { _count: { select: { scenes: true, generationTasks: true } } },
  });
...
export async function POST(request: NextRequest) {
  ...
  if (!parsed.success) return NextResponse.json({ error: "Informations du projet invalides.", details: parsed.error.flatten().fieldErrors }, { status: 400 });
