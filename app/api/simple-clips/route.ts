...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    // Refus avant toute écriture et avant toute réservation de crédits.
    if (!worker.available || !worker.paidGenerationAllowed) {
      return NextResponse.json({
        code: worker.paidGenerationRefusal ?? "WORKER_UNAVAILABLE",
        error: PAID_GENERATION_UNAVAILABLE_MESSAGE,
      }, { status: 503 });
    }
